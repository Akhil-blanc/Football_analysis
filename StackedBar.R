...
# # Function to scrape individual match URLs
# scrape_match <- function(url) {
#   tryCatch(
#     {
#       df <- .get_clean_understat_json(page_url = url, script_name = "shotsData")
#       if(nrow(df) == 0) {
#         print(glue::glue("Shots data for match_url {url} not available"))
#         return(NULL)
#       }
#       return(df)
#     },
#     error = function(e) {
#       print(glue::glue("Error scraping {url}: {conditionMessage(e)}"))
#       return(NULL)
#     }
#   )
# }

# # Modified function for scraping Understat shooting data
# understat_shooting_parallel <- function(type_url) {
#   main_url <- "https://understat.com/"
  
#   # Get the game IDs first, filtering out matches not yet played
#   games <- .get_clean_understat_json(page_url = type_url, script_name = "datesData") %>%
#     dplyr::filter(isResult)
  
#   # Create a vector of match URLs
#   match_urls <- paste0(main_url, "match/", games$id)
  
#   # Scrape match URLs in parallel
#   shots_data <- future_map_dfr(match_urls, ~scrape_match(.x))
